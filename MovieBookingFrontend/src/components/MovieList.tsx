...
const MovieList: React.FC = () => {
  ...
  const deleteMovie = async (movieName: string): Promise<void> => {
    try {
      ...
      const updatedMovies = movies.filter((movie) => movie.movieName !== movieName);
      setMovies(updatedMovies);
      setFilteredMovies(updatedMovies);
    } catch (error: any) {
      if (error.response?.data?.message?.includes('Required request header')) {
        setErrorMessage('You are not logged in. Please login before proceeding.');
      } else {
  ...
  const genres = [...new Set(movies.map(movie => movie.genre?.split(',')[0]?.trim()).filter(Boolean))];
